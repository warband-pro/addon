...
local function cell(text, tone)
  if text == nil then return nil end
  return { text = text, tone = tone or "plain" }
end
...
local function thisWeek(groups, cols)
  ...
  for _, v in ipairs(VAULT) do
    addRow(g, cols, v.label, function(c)
      ...
      if not t then return cell(unlocked .. " slots", "good") end
      local text = format("%d/%d", num(b.progress) or 0, t)
      if unlocked > 0 then text = text .. format(" (%d)", unlocked) end
      return cell(text, unlocked > 0 and "good" or "plain")
    end)
  end
    ...
    addRow(g, cols, k.label, function(c)
      for _, inst in ipairs(c.instances or {}) do
        if inst.name and (inst.name .. "|" .. (inst.difficultyName or "")) == k.key then
          ...
          if total == 0 then return cell("saved", "warn") end
          return cell(format("%d/%d", killed, total), killed >= total and "good" or "warn")
        end
      end
      return nil
    end)
      ...
      for _, cur in ipairs(c.currencies or {}) do
        if cur.id == k.id then
          ...
          if max and max > 0 then
            return cell(commas(q) .. "/" .. commas(max), q >= max * 0.9 and "warn" or "plain")
          end
          return cell(commas(q), "plain")
        end
      end
      return nil
    end)
