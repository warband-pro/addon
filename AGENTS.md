...
`CLAUDE.md` is a pointer to this file for Claude Code compatibility — one
contract, no drift. Claude-specific notes live in `.claude/` and the supplement
in `CLAUDE.md`.
...
commands CI uses, takes about 40 seconds, and agent tooling is configured to allow
both without asking:
...
2026-09-22, all four green on Windows Node.
...
repo commit hook treats a `.lua` file with a slash in its path as a file
...
- **Agent permission config is a convenience, not a boundary.** The allow list
...
A pre-tool-use commit hook (Claude Code implementation below)
...
delete a remote ref from inside an agent web session** — it is refused at
...
and some agent GitHub tooling offers `create_branch` with no delete counterpart.
